...
def SocketThread():
    ...
    print(client.connect(('192.168.43.250', 6666)))  # 链接服务器的ip + 端口
        #time.sleep(10)

    #con.acquire()
    while True:
        if len(sendstrbuff) == 0:
            continue
        print("sendstrbuff:",sendstrbuff)
        print(client.send(sendstrbuff.encode("utf-8")))
        #time.sleep(10)
        print("sending...")
       # con.notify()  # 唤醒其它线程
        #con.wait()
    #con.release()
#add by wzj 20210810 end
...
if __name__ == '__main__':
    ...
    # client = socket.socket()  # 声明socket类型，同时生成socket连接对象 clinet is 201, it uses the car
    # client.connect(('192.168.43.250', 6666))  # 链接服务器的ip + 端口
    #t=threading.Thread(target=SocketThread,args=('4',))
    t=threading.Thread(target=SocketThread,args=())
    t.start()
    #soc = SocketThread()
    #soc.start()
